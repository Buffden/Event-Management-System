      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Button
            ...
            type="button"
            onClick={() => {
              logger.userAction(LOGGER_COMPONENT_NAME, 'Google OAuth login initiated');
              // Redirect to OAuth endpoint (no role needed for login, existing users keep their role)
              const authBaseUrl = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost/api';
              const oauthUrl = `${authBaseUrl}/auth/google`;
