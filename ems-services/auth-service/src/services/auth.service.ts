...
class AuthService {
    ...
    async sendAccountSuspendedEmail(user: User): Promise<void> {
        try {
            logger.debug("START - sendAccountSuspendedEmail(): ", {userId: user.id});

            const msg = {
                type: MESSAGE_TYPE.ACCOUNT_SUSPENDED,
                message: {
                    to: user.email,
                    subject: `${process.env.APP_NAME || 'EVENTO'} Account Suspended`,
                    body: '', // Will be generated by email template service
                    userName: user.name || 'User',
            ...
            const queueName = 'notification.email';
            await rabbitMQService.sendMessage(queueName, msg);
            logger.debug("END - sendAccountSuspendedEmail(): ", {userId: user.id});
        } catch (error) {
            logger.error("sendAccountSuspendedEmail(): Failed to publish account suspended email message", error as Error, {userId: user.id});
            // Don't throw error - email failure shouldn't prevent suspension
        }
    }

    async sendAccountUnsuspendedEmail(user: User): Promise<void> {
        try {
            logger.debug("START - sendAccountUnsuspendedEmail(): ", {userId: user.id});

            const dashboardLink = `${process.env.CLIENT_URL || 'http://localhost'}/dashboard/${user.role === 'SPEAKER' ? 'speaker' : 'attendee'}`;

            const msg = {
                type: MESSAGE_TYPE.ACCOUNT_UNSUSPENDED,
                message: {
                    to: user.email,
                    subject: `${process.env.APP_NAME || 'EVENTO'} Account Reinstated`,
                    body: '', // Will be generated by email template service
                    userName: user.name || 'User',
            ...
            const queueName = 'notification.email';
            await rabbitMQService.sendMessage(queueName, msg);
            logger.debug("END - sendAccountUnsuspendedEmail(): ", {userId: user.id});
        } catch (error) {
            logger.error("sendAccountUnsuspendedEmail(): Failed to publish account unsuspended email message", error as Error, {userId: user.id});
            // Don't throw error - email failure shouldn't prevent unsuspension
        }
    }
