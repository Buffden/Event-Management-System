// src/consumers/event-status.consumer.ts
...
export class EventStatusConsumer {
  ...
  constructor(rabbitmqUrl: string) {
    ...
    this.speakerServiceUrl = `${gatewayUrl}/api/speaker`;
  }

  public async start(): Promise<void> {
    try {
      console.log('🚀 Starting Event Status Consumer...');
      ...
      if (this.channel) {
        ...
        console.log(`👂 Worker listening for event status changes on exchange "${this.exchangeName}" with routing key "${this.routingKey}"`);
        this.channel.consume(this.queueName, this.handleMessage.bind(this), { noAck: false });
      }
    } catch (error) {
      console.error('❌ Error starting event status consumer:', error);
      setTimeout(() => {
  ...
  private async handleMessage(msg: ConsumeMessage | null) {
    ...
    try {
      ...
      console.log('📧 Processing event cancelled event:', eventData);
  ...
  private async processEventCancelledEvent(eventData: EventCancelledMessage) {
    try {
      ...
      if (!eventInfo) {
        console.error('❌ Failed to fetch event info for event cancellation notifications');
        return;
      }
      ...
      // Get all speaker invitations for this event
      const invitations = await this.getEventInvitations(eventData.eventId);
      const invitationCount = invitations?.length || 0;

      if (bookingCount === 0 && invitationCount === 0) {
        console.log(`ℹ️  No confirmed bookings or speaker invitations found for cancelled event ${eventData.eventId}`);
        return;
      }

      console.log(`📧 Sending cancellation notifications to ${bookingCount} registered users and ${invitationCount} invited speakers`);
      ...
      if (bookings && bookings.length > 0) {
        for (const booking of bookings) {
          try {
            ...
            if (!userInfo) {
              console.warn(`⚠️  Failed to fetch user info for userId: ${booking.userId}`);
              continue;
            }
            ...
            console.log(`✅ Event cancellation notification queued for attendee ${userInfo.email}`);
          } catch (error) {
            console.error(`❌ Error sending notification to user ${booking.userId}:`, error);
            // Continue with other users even if one fails
          }
        }
      }

      // Send notification to each invited speaker
      if (invitations && invitations.length > 0) {
        for (const invitation of invitations) {
          try {
            // Fetch speaker profile to get email
            const speakerInfo = await this.getSpeakerInfo(invitation.speakerId);
            if (!speakerInfo) {
              console.warn(`⚠️  Failed to fetch speaker info for speakerId: ${invitation.speakerId}`);
              continue;
            }

            // Create event cancelled notification for speaker
            const notification: EventCancelledNotification = {
              type: MESSAGE_TYPE.EVENT_CANCELLED_NOTIFICATION,
              message: {
                ...
                subject: '', // Will be generated by email template service
                body: '',    // Will be generated by email template service
                attendeeName: speakerInfo.name || speakerInfo.email,
                eventName: eventInfo.name,
                eventDate: new Date(eventInfo.bookingStartDate).toLocaleDateString(),
                venueName: eventInfo.venue?.name || 'Unknown Venue',
                cancellationReason: undefined, // Can be added later if needed
            ...
            console.log(`✅ Event cancellation notification queued for speaker ${speakerInfo.email}`);
          } catch (error) {
            console.error(`❌ Error sending notification to speaker ${invitation.speakerId}:`, error);
            // Continue with other speakers even if one fails
          }
        }
      }

      console.log(`✅ Completed processing event cancellation notifications for ${eventData.eventId}`);
    } catch (error) {
      console.error('❌ Error processing event cancelled event:', error);
    }
  }
  ...
  private async getEventInvitations(eventId: string): Promise<Array<{ speakerId: string }> | null> {
    try {
      const response = await axios.get(`${this.speakerServiceUrl}/internal/invitations/event/${eventId}`, {
        timeout: 5000,
        headers: {
          'Content-Type': 'application/json',
          'x-internal-service': 'notification-service'
      ...
      if (response.status === 200 && response.data.success) {
        const invitations = response.data.data.map((invitation: any) => ({
  ...
  private async getSpeakerInfo(speakerId: string): Promise<{ email: string; name?: string } | null> {
    try {
      const response = await axios.get(`${this.speakerServiceUrl}/internal/speakers/${speakerId}`, {
        timeout: 5000,
        headers: {
          'Content-Type': 'application/json',
          'x-internal-service': 'notification-service'
  ...
  public async stop(): Promise<void> {
      console.log('🔌 Shutting down event status consumer...');
    try {
      if (this.channel) await this.channel.close();
      if (this.connection) await this.connection.close();
    } catch (error) {
      console.error('Error during event status consumer shutdown:', error);
    }
  }
}
