// src/consumers/event-updated.consumer.ts

import { connect, ChannelModel, Channel, ConsumeMessage } from 'amqplib';
import axios from 'axios';
import { MESSAGE_TYPE, EventUpdatedNotification } from '../types/types';
...
export class EventUpdatedConsumer {
  private connection: ChannelModel | null = null;
  private channel: Channel | null = null;
  private readonly rabbitmqUrl: string;
  private readonly exchangeName = 'event.exchange';
  private readonly queueName = 'notification.event.updated';
  private readonly routingKey = 'event.updated';
  ...
  public async start(): Promise<void> {
    try {
      console.log('🚀 Starting Event Updated Consumer...');
      ...
      if (this.channel) {
        ...
        console.log(`👂 Worker listening for event updated events on exchange "${this.exchangeName}" with routing key "${this.routingKey}"`);
        this.channel.consume(this.queueName, this.handleMessage.bind(this), { noAck: false });
      }
    } catch (error) {
      console.error('❌ Error starting event updated consumer:', error);
      setTimeout(() => {
  ...
  private async handleMessage(msg: ConsumeMessage | null) {
    ...
    try {
      const eventData: EventUpdatedMessage = JSON.parse(msg.content.toString());
      console.log('📧 Processing event updated event:', eventData);

      // Process the event updated event
      await this.processEventUpdatedEvent(eventData);

      // Acknowledge the message
      this.channel.ack(msg);
    } catch (error) {
      console.error('❌ Error processing event updated event:', error);
      this.channel.nack(msg, false, false);
    }
  }

  private async processEventUpdatedEvent(eventData: EventUpdatedMessage) {
    try {
      // Fetch event details
      const eventInfo = await this.getEventInfo(eventData.eventId);
      if (!eventInfo) {
        console.error('❌ Failed to fetch event info for event update notifications');
        return;
      }
      ...
      if (!bookings || bookings.length === 0) {
        console.log(`ℹ️  No confirmed bookings found for updated event ${eventData.eventId}`);
        return;
      }

      // Convert updatedFields object keys to a readable array
      const updatedFields = Object.keys(eventData.updatedFields).map(field => {
      ...
      console.log(`📧 Sending update notifications to ${bookings.length} registered users`);
      ...
      for (const booking of bookings) {
        try {
          ...
          const notification: EventUpdatedNotification = {
          ...
          console.log(`✅ Event update notification queued for ${userInfo.email}`);
        } catch (error) {
      ...
      console.log(`✅ Completed processing event update notifications for ${eventData.eventId}`);
    } catch (error) {
      console.error('❌ Error processing event updated event:', error);
    }
  }
  ...
  public async stop(): Promise<void> {
    console.log('🔌 Shutting down event updated consumer...');
    try {
      if (this.channel) await this.channel.close();
      if (this.connection) await this.connection.close();
    } catch (error) {
      console.error('Error during event updated consumer shutdown:', error);
    }
  }
}
